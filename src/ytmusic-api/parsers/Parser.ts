...
export default class Parser {
    ...
    public static parseMixedContent(data: any): HomePageContent | null {
        ...
        const musicDescriptionShelfRenderer = traverse(
            result,
        ...
        if (
            musicDescriptionShelfRenderer &&
            !Array.isArray(musicDescriptionShelfRenderer)
        ) {
            return {
                title: traverse(
                    musicDescriptionShelfRenderer,
        ...
        result.contents.forEach((content: any) => {
            const musicTwoRowItemRenderer = traverse(
                content,
                'musicTwoRowItemRenderer'
            );
            if (
                musicTwoRowItemRenderer &&
                !Array.isArray(musicTwoRowItemRenderer)
            ) {
                ...
                switch (pageType) {
                    ...
                    case PageType.MUSIC_PAGE_TYPE_PLAYLIST:
                        contents.push(
                            PlaylistParser.parseSearchResult(content)
                        );
                        break;
                    default:
                        if (playlistId) {
                            contents.push(
                                PlaylistParser.parseWatchPlaylist(content)
                            );
                        } else {
                            contents.push(
                                SongParser.parseSearchResult(content)
                            );
                        }
                }
            } else {
